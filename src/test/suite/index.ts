...
//@ts-ignore TO DO - figure out how nyc actually works and fix this error
...
export async function run(): Promise<void> {
  ...
  const nyc = new NYC({
    ...
    include: [ "out/**/*.js", "csvEditorHtml/out/*.js" ],
    exclude: [ "out/test/**" ],
  });
  await nyc.wrap();
  ...
  const myFilesRegex = /vscode-tabular-yaml\/out/;
...
async function captureStdout(fn: any) {
  ...
  process.stdout.write = (s: any) => { buffer = buffer + s; return true; };
  await fn();
